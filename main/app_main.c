        ...
        if (button_state != old_button_state && button_state == 0) {
            ...
            if (current_time - last_button_press_time > pdMS_TO_TICKS(1000)) {
                ...
                if (button_press_count == 2) {
                    ...
                    xQueueSend(button_queue, &led_state_1, 0);  // Send LED state to the other task
                } 
                else if (button_press_count == 4) {
                    int smart_config = 1;
                    xQueueSend(button_queue, &smart_config, 0);  // Send LED state to the other task
                    printf("4 times\n");
...
void led_task(void* parameter) {
    ...
    while (1) {
        if (xQueueReceive(button_queue, &led_state_1, portMAX_DELAY) == pdTRUE) {
...
void smart_config(void* parameter) {
    int smart_config = 0;
    while (1) {
        if (xQueueReceive(button_queue, &smart_config, portMAX_DELAY) == pdTRUE) {
            EventBits_t bits = xEventGroupGetBits(s_wifi_event_group);
...
void app_main(void)
{
    ...
    //tao 1 queue de xu ly nut nhan
    button_queue = xQueueCreate(QUEUE_LENGTH, ITEM_SIZE);
